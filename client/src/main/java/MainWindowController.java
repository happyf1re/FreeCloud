...
public class MainWindowController implements Initializable {
    ...
    @FXML
    private Button clientRootButton;
    ...
    @FXML
    private Button clientNewdirButton;

    @FXML
    private Label clientCurrentPath;

    @FXML
    private ListView<StorageItem> clientListView;

    @FXML
    private Button serverRootButton;
    ...
    @FXML
    private Button serverNewdirButton;

    @FXML
    private Label serverCurrentPath;

    @FXML
    private ListView<StorageItem> serverListView;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        mainWindowServerListener.setDaemon(true);
        mainWindowServerListener.start();
        deleteChosenFilesOnKeyDeletePressed();
        updateCloudStoragePanel();
    }

    Thread mainWindowServerListener = new Thread(new Runnable() {
        @Override
        public void run() {
            try {
                while (true) {
    ...
    public void initializeListOfLocalStorageItems() {
        ...
        if (listOfLocalStorageFiles.length == 0 && localStorageFolderLevelCounter == 0) {
            ...
            clientListView.setBackground(new Background(bi));
            clientListView.setOpacity(0.9);
            clientListView.setItems(listOfLocalItems);
            clientListView.setCellFactory(param -> new StorageListViewItem());
        } else if (listOfLocalStorageFiles.length > 0){
            clientListView.setBackground(null);
    ...
    public void initializeListOfCloudStorageItems(HashMap<Integer, LinkedList<File>> listOfCloudStorageFiles) {
        if (cloudStorageFolderLevelCounter > 0){
            ...
            clientUpButton.setVisible(false);
        }
        try {
            ...
            if (!listOfCloudStorageFiles.isEmpty()) {
                ...
                serverListView.setItems(listOfCloudItems);
                serverListView.setCellFactory(param -> new StorageListViewItem());
            } else {
                serverListView.setItems(listOfCloudItems);
                serverListView.setCellFactory(param -> new StorageListViewItem());
            }
    ...
    public void dragAndDropIntoLocalStorage() {
        clientListView.setOnDragOver(event -> {
            clientListView.setStyle("-fx-background-color: white; -fx-opacity: 0.5;");
        ...
        clientListView.setOnDragDropped(event -> {
            ...
            if (db.hasFiles()) {
                for (int i = 0; i < db.getFiles().size(); i++) {
                    ...
                    clientListView.setItems(listOfFiles);
                    clientListView.setCellFactory(param -> new StorageListViewItem());
                }
                initializeListOfLocalStorageItems();
                clientListView.setStyle("-fx-background-color: white;");
            }
            success = true;
        ...
        clientListView.setOnDragExited(event -> {
            clientListView.setStyle("-fx-background-color: white; -fx-opacity: 1;");
            event.acceptTransferModes(TransferMode.NONE);
        });
    }
    public void dragFilesFromLocalStorageToCloud() {
        clientListView.setOnDragDetected(event -> {
            Dragboard db = clientListView.startDragAndDrop(TransferMode.COPY);
    ...
    public void goToNextDirectoryInLocalStorageOnDoubleClickOrOpenFile(MouseEvent mouseEvent) {
        if (mouseEvent.getClickCount() == 1) {
            ...
            if (clientListView.getSelectionModel().getSelectedItems().size() == 1) {
                File pathToClickedFile;
                pathToClickedFile = clientListView.getSelectionModel().getSelectedItem().getPathToFile();
                if (pathToClickedFile.isDirectory()) {
                    ...
                    if (nextDirectory.length == 0) {
                        ...
                        if (localStorageFolderLevelCounter > 0) {
                            clientUpButton.setVisible(true);
                        }
    ...
    public void goToNextDirectoryInCloudStorageOnDoubleClick(MouseEvent mouseEvent) {
        ...
        if (mouseEvent.getClickCount() == 1) {
            ...
            if (serverListView.getSelectionModel().getSelectedItems().size() == 1) {
                ...
                for (int i = 0; i < folderCloudStorageListViews.get(cloudStorageFolderLevelCounter).size(); i++) {
                    ...
                    if (serverListView.getSelectionModel().getSelectedItem().getName().equals(file.getName())) {
                ...
                if (pathToClickedFile.isDirectory()) {
                    ...
                    if (cloudStorageFolderLevelCounter > 0) {
                        serverUpButton.setVisible(true);
                    }
                }else {
    ...
    public void goToPreviousDirectoryInCloudStorage(ActionEvent event) {
        ...
        cloudStorageFolderLevelCounter--;
        if (cloudStorageFolderLevelCounter <= 0) {
            serverUpButton.setVisible(false);
        }
    }
    ...
    public LinkedList getPathsOfSelectedFilesInLocalStorage() {
        try {
            ...
            if (clientListView.getSelectionModel().getSelectedItems().size() != 0) {
                System.out.println(clientListView.getSelectionModel().getSelectedItems().size());
                for (int i = 0; i < clientListView.getSelectionModel().getSelectedItems().size(); i++) {
                    listOfSelectedElementsInLocalStorage.add(clientListView.getSelectionModel().getSelectedItems().get(i).getPathToFile());
                }
    ...
    public LinkedList getPathsOfSelectedFilesInCloudStorage(){
        ...
        if (serverListView.getSelectionModel().getSelectedItems().size() != 0){
            for (int i = 0; i < serverListView.getSelectionModel().getSelectedItems().size(); i++) {
                listOfSelectedElementsInCloudStorage.add(serverListView.getSelectionModel().getSelectedItems().get(i).getPathToFile());
            }
        }
    ...
    public void deleteChosenFilesOnKeyDeletePressed() {
        ...
        serverListView.setOnKeyPressed(event -> {
    ...
    public void selectAllFilesFromCloudStorage() {
        if (serverListView.getItems().size() == serverListView.getSelectionModel().getSelectedItems().size()) {
            serverListView.getSelectionModel().clearSelection();
        } else {
            serverListView.getSelectionModel().setSelectionMode(SelectionMode.MULTIPLE);
            serverListView.getSelectionModel().selectAll();
        }
    }
    public void selectAllFilesFromLocalStorage() {
        if (clientListView.getItems().size() == clientListView.getSelectionModel().getSelectedItems().size()) {
            clientListView.getSelectionModel().clearSelection();
        } else {
            clientListView.getSelectionModel().setSelectionMode(SelectionMode.MULTIPLE);
            clientListView.getSelectionModel().selectAll();
        }
    }
    ...
    public void createDirectoryOnLocalStorage() {
        File newDirectory = new File("client/LocalStorage/NewDir");
        newDirectory.mkdir();
    }
